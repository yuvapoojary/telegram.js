...
class CallbackQuery extends Base {
  ...
  _patch(data) {
    ...
    if ('message' in data) {
      /**
       * The message with the callback button that originated the query. Note that message content and message date will not be available if the message is too old
       * @type {?Message}
       */
    ...
    if ('chat_instance' in data) {
      /**
       * Global identifier, uniquely corresponding to the chat to which the message with thehort name of a Game to be returned, serves as the unique identifier for the game callback button was sent. Useful for high scores in games.
       * @type {?string}
       */
   ...
   * @param {boolean} [options.show_alert] Whether to show an alert instead of notification
   * @param {string} [url] URL that will be opened by the user's client
   * @param {number} [cache_time] The maximum amount of time in seconds that the result of the callback query may be cached client-side.Defaults to 0.
   */
